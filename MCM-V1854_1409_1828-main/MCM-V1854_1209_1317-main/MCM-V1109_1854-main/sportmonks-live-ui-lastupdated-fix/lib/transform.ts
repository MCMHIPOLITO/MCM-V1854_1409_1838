export function transformFixture(fix:any){
  ...
  // Scores
  const scoreHome = (fix?.scores??[]).find((s:any)=>s.description==='CURRENT' && s.participant_id==homeId)?.score?.goals ?? 0;
  const scoreAway = (fix?.scores??[]).find((s:any)=>s.description==='CURRENT' && s.participant_id==awayId)?.score?.goals ?? 0;
  ...
  function maxDangerousForPeriod(periodId:number, pid:number){
    const arr=(fix?.trends??[]).filter((x:any)=>x.type_id==44 && x.participant_id==pid && x.period_id==periodId);
    if(arr.length===0) return 0;
  ...
  // Derived
  const crossBlocked={home:Math.max(0,crosses.home-crossAcc.home),away:Math.max(0,crosses.away-crossAcc.away)};
  ...
  return {
    matchName,minute,periodLabel,scoreHome,scoreAway,
    corners,attacks,dangerous,dangerous1HT,dangerous2HT,dangerousAdjusted,
    possession,shotsOnTarget,totalShots,blockedShots,crosses,crossAcc,reds,crossBlocked,
    speedH,speedA,speedSum,blockedValueHome,blockedValueAway,
    speedBlockedHome,speedBlockedAway,blockedAcum
  };
}
...
export function flattenRow(tf:any){
  const row:any={
    ...
    "Corners\nHome":tf.corners.home,"Corners\nAway":tf.corners.away,
    "Attacks\nHome":tf.attacks.home,"Attacks\nAway":tf.attacks.away,
    "Dangerous Attacks\nHome":tf.dangerousAdjusted.home,"Dangerous Attacks\nAway":tf.dangerousAdjusted.away,
    "Dangerous Attacks 1HT\nHome":tf.dangerous1HT.home,"Dangerous Attacks 1HT\nAway":tf.dangerous1HT.away,
    "Dangerous Attacks 2HT\nHome":tf.dangerous2HT.home,"Dangerous Attacks 2HT\nAway":tf.dangerous2HT.away,
    "Ball Possession\nHome %":tf.possession.home,"Ball Possession\nAway %":tf.possession.away,
    "Shots On Target\nHome":tf.shotsOnTarget.home,"Shots On Target\nAway":tf.shotsOnTarget.away,
    "Total Shots\nHome":tf.totalShots.home,"Total Shots\nAway":tf.totalShots.away,
    "Blocked Shots\nHome":tf.blockedShots.home,"Blocked Shots\nAway":tf.blockedShots.away,
    "Crosses\nHome":tf.crosses.home,"Crosses\nAway":tf.crosses.away,
    "Cross Accuracy\nHome":tf.crossAcc.home,"Cross Accuracy\nAway":tf.crossAcc.away,
    "Crosses blocked\nHome":tf.crossBlocked.home,"Crosses blocked\nAway":tf.crossBlocked.away,
    "Red Card\nHome":tf.reds.home,"Red Card\nAway":tf.reds.away,
    "Speed APPM\nH":tf.speedH,"Speed APPM\nA":tf.speedA,"Speed\nAcum":tf.speedSum,
    "Blocked Value\nHome":tf.blockedValueHome,"Blocked Value\nAway":tf.blockedValueAway,
    "Speed Blocked\nHome":tf.speedBlockedHome,"Speed Blocked\nAway":tf.speedBlockedAway,
    "Blocked\nAcum":tf.blockedAcum
  };
  const { WPI_HOME, WPI_AWAY } = calculateWPI(
    row["Crosses Home"] || 0,
    row["Crosses Away"] || 0,
    row["Total Shots Home"] || 0,
    row["Total Shots Away"] || 0
  );
